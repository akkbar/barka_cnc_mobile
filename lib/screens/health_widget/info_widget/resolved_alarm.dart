...
class ResolvedAlarmTable extends StatefulWidget {
  final int machineId;

  const ResolvedAlarmTable({super.key, required this.machineId});

  @override
  _ResolvedAlarmTableState createState() => _ResolvedAlarmTableState();
}

class _ResolvedAlarmTableState extends State<ResolvedAlarmTable> {
  ...
  Future<void> fetchAlarmData() async {
    try {
      ...
      final response = await http.get(Uri.parse('$apiUrl/api/resolved?machineId=${widget.machineId}'));
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to load alarm data: ${response.statusCode}'); // Log the status code
      ...
      child: Column(
        children: [
          const Padding(
            ...
            child: Text("Resolved Alarms", style: TextStyle(fontWeight: FontWeight.bold)),
          ),
